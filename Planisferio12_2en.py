...
class Chanubih:
    ...
    def __init__(self, parent,ancho_p, alto_p):
        ...
        lab2_cred = Label(row_cred, width = 126, text=u'Credits: (\u037B) 2017 Eduardo Rodas')
        lab2_cred.pack()
        ...
        row6_fecha = Button(self.fecha, text = "Confirm data",command = evaluar, background="#B0BEC5")
        row6_fecha.bind("<Return>", evaluar_e)

        lab1_fecha = Label(row1_fecha, width=40, text="DATE", background = color_fecha_label)
...
#        row5_lugar = Frame(self.lugar, highlightthickness = 0)
        ...
        row8_lugar = Button(self.lugar, text = "Confirm data", command = evaluarl, background="#B0BEC5")

        lab1_lugar = Label(row1_lugar, width=39, text="LOCATION", background = color_lugar_label)
...
#        lab5_lugar = Label(row5_lugar, width=40, text=u"    Formato: \u00B1ggg:mm:ss", background = color_lugar)
        ...
        row4_alinmto = Button(self.alinmto, text = "Confirm data", command = evaluarA, background="#B0BEC5")
        row4_alinmto.bind=('<Return>',evaluarA_e)

        lab1_alinmto = Label(row1_alinmto, width=39, text="ALIGNMENT TO EVALUATE", background = color_lugar_label)
        lab2_alinmto = Label(row2_alinmto, width=19, text=u"Azimuth(\u00B1dd.xx):", background = color_lugar)
        ...
        row2_otrosf.configure (text = "Coincidence\nSun-Azimuth", background="#90A4AE", width = 8)
        row3_otrosf = Button(self.otrosf, command = self.decsluna)
        row3_otrosf.configure (text = "Lunar\nStandstills", background="#90A4AE", width = 6)
        ...
        row5_otrosf.configure (text = "Sun passing\nthrough Zenith", background="#90A4AE", width = 7)

        lab1_otrosf = Label(row1_otrosf, width=40, text="CALCULATE TOPOCENTRICAL EVENTS", background = color_otrosf_label)
...
############# ACIMUT EVALUADO Y ACIMUT DEL SOL, CUANDO ESTA DIFERENCIA ES MINIMA, OCURRE COINCIDENCIA ###############
        ...
        b1 = Button(obtalt, text='Confirm data', command=(lambda altura=ents: retornar(altura)))
        b1.focus_force()
        b1.pack(side=LEFT, padx=5, pady=5)
        b1.bind('Return', (lambda event, altura=ents: retornar(altura)))
        b2 = Button(obtalt, text='Cancel', command=salir)
        b2.pack(side=RIGHT, padx=5, pady=5)

        obtalt_xpos=int((screen_width - wwidth * 5 - 10*5) / 2)
        obtalt_ypos=int((screen_height * 0.5))
        obtalt.title("Computes dates that Sun rises in azimuth of structure")
...
#######################################
    ...
    def graficar(self, n_cuerpoc,fig, ax, canvas):
        ...
        ax.text(-0.13,0.96, 'Rise of '+n_cuerpoc+': '+str(k_cuerpoc_sale)+' (UT)',
            horizontalalignment='left',
            verticalalignment='top',
        ...
        ax.text(-0.13,0.94, 'Crosses meridian: '+str(lugarc.next_transit(cuerpoc))+"(UT)",
            horizontalalignment='left',
            verticalalignment='top',
            fontsize=7.5, color='black',
            transform=ax.transAxes)

        ax.text(-0.13,0.92, 'Max. height above horizon: '+str(cuerpoc.alt),
            horizontalalignment = 'left',
            verticalalignment = 'top',
            fontsize = 7.5, color = 'black',
            transform = ax.transAxes)
        ...
        ax.text(-0.13,0.90, 'Evaluated Azimuth:' + str(aci),
            horizontalalignment = 'left',
            verticalalignment = 'top',
            fontsize = 7.5, color='black',
            transform = ax.transAxes)
